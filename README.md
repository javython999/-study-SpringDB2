...
스프링 데이터 JPA는 JPA를 편리하게 사용할 수 있도록 도와주는 라이브러리다. 많은 기능을 제공하지만
대표적인 기능은 다음과 같다.
* 공통 인터페이스 기능
...
***
# 데이터 접근 기술 - Querydsl
스프링 데이터 JPA를 통해 JPA를 쉽게 사용할 수 있게 되었지만, 동적 쿼리 작성은 복잡하기도하고
SQL과 비슷하지만 JPQL로 작성해야하는 문제가 있다.
JPQL은 String 변수에 작성한다. 오타나 띄어쓰기가 잘못 되었을 경우 개발자가 놓치면 런타임에러가 발생하게 된다.
이러한 문제점을 해결 할 수 있는 기술이 Querydsl이다. Querydsl을 통해 자바 코드로 쿼리를 작성하면 이걸 JPQL로 자동으로 변환해 처리해준다.

* 장점
  * 문자가 아닌 자바 코드로 쿼리를 작성하기 때문에 잘못된 점이 있을 경우 컴파일 시점에 오류를 확인 할 수 있다.
  * 자바 코드로 작성하기 때문에 IDE의 자동완성 기능의 도움을 받을 수 있다.
  * 동적쿼리 작성시 쿼리 조건등을 메서드로 만들어 재사용성이 높다.
* 단점
  * Querydsl은 컴파일 단계에서 엔티티를 기반으로 Q클래스 파일들을 생성하게 된다. IDE에 따라 이와 관련된 설정을 해주어야 한다.(빌드전 기존 Q클래스 삭제, 경로 지정 등)
***
